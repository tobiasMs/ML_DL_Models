...
# =========================================================
# 14. EXPORT TXT JURNAL-FRIENDLY (UPDATED FOR 300x300)
# =========================================================
...
def export_journal_friendly_report():
    ...
    with open(REPORT_TXT_PATH, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"Input images were resized to {IMAGE_SIZE[0]} x {IMAGE_SIZE[1]} x 3 and augmented during training. "
        ...
        f.write("----------------------------------------\n")
        f.write(
            f"This study performed binary classification of tomato leaf images into diseased and healthy classes. "
            f"The dataset consisted of 6,000 images, with 3,000 images assigned to each class. "
            f"A stratified train-validation split was applied with an 80:20 ratio, resulting in 4,800 training images "
            f"and 1,200 validation images. The validation set contained 600 diseased and 600 healthy images. "
            f"Images were resized to {IMAGE_SIZE[0]} x {IMAGE_SIZE[1]} pixels and processed using a TensorFlow data pipeline. "
            f"Data augmentation, including random horizontal flipping, rotation, zooming, and contrast adjustment, "
            f"was applied during training to improve generalization. The model used ResNet50 pretrained on ImageNet "
            f"as the feature extractor, followed by a custom classifier head consisting of global average pooling, "
            f"batch normalization, dense layers, dropout, and a sigmoid output layer. Training was conducted in two phases: "
            f"first, only the classifier head was trained while the base model was frozen; second, the last 50 layers "
            f"of the ResNet50 base model were fine-tuned with a lower learning rate. The model was optimized using Adam "
            f"and binary cross-entropy loss. Performance was evaluated using accuracy, precision, recall, F1-score, AUC, "
            f"and confusion matrix analysis."
        )
        f.write("\n\n")
